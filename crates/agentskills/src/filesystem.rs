...
mod tests {
    ...
    #[test]
    fn test_memfs_file_name() {
        let fs = MemFS::new();
        let path = Path::new("/path/to/file.txt");

        assert_eq!(fs.file_name(path), Some("file.txt".to_string()));
    }

    #[test]
    fn test_memfs_parent() {
        let fs = MemFS::new();
        let path = Path::new("/path/to/file.txt");

        assert_eq!(fs.parent(path), Some(PathBuf::from("/path/to")));
    }
    ...
    fn test_memfs_multiple_files() {
        ...
        assert_eq!(
            fs.read_to_string(Path::new("/skill1/SKILL.md")).unwrap(),
            "skill 1"
        );
        assert_eq!(
            fs.read_to_string(Path::new("/skill2/SKILL.md")).unwrap(),
            "skill 2"
        );
        assert_eq!(
            fs.read_to_string(Path::new("/skill3/SKILL.md")).unwrap(),
            "skill 3"
        );
    }
